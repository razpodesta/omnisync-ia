...
export class QdrantDriver implements IVectorDatabaseAgnosticDriver {
  ...
  public async executeSemanticSearch(
    ...
    const collectionAlias = QdrantCollectionApparatus.resolveAlias(tenantOrganizationIdentifier);

    return await OmnisyncTelemetry.traceExecution(apparatusName, 'executeSemanticSearch', async () => {
      /**
      ...
      return await OmnisyncSentinel.executeWithResilience(
        async () => {
          const response = await fetch(`${this.technicalConfiguration.endpoint}/collections/${collectionAlias}/points/search`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'api-key': this.technicalConfiguration.apiKey },
            body: JSON.stringify({
          ...
          const payload = await response.json() as { result: IQdrantInternalPoint[] };

          return payload.result.map(point =>
            QdrantMapperApparatus.mapToOmnisyncChunk(point, tenantOrganizationIdentifier)
          );
        },
        apparatusName,
  ...
  public async upsertKnowledgeChunks(knowledgeChunks: IKnowledgeSemanticChunk[]): Promise<void> {
    ...
    return await OmnisyncTelemetry.traceExecution(apparatusName, 'upsertKnowledgeChunks', async () => {
      ...
      const points = knowledgeChunks.map(chunk => QdrantMapperApparatus.mapToQdrantPoint(chunk));

      await OmnisyncSentinel.executeWithResilience(
        async () => {
          const res = await fetch(`${this.technicalConfiguration.endpoint}/collections/${collectionAlias}/points`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'api-key': this.technicalConfiguration.apiKey },
