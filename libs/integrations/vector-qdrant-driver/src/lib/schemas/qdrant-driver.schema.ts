 ...
 */
export const QdrantConnectionConfigurationSchema = z.object({
  ...
  apiKey: z.string().min(10, { message: 'Qdrant API Key insuficiente o inexistente.' }),
}).readonly();

export type IQdrantConnectionConfiguration = z.infer<typeof QdrantConnectionConfigurationSchema>;

/**
 * @name QdrantInternalPointSchema
 * @description Contrato para la validación de la respuesta cruda del motor Rust de Qdrant.
 */
export const QdrantInternalPointSchema = z.object({
  id: z.string().uuid(),
  score: z.number(),
