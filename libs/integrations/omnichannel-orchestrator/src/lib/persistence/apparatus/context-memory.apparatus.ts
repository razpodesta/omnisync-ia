 ...
 */
import { ConversationFragmentSchema, IConversationFragment } from '../schemas/context-memory.schema';
...
export class ContextMemoryApparatus {
  ...
  public static async pushConversationFragment(
    ...
    return await OmnisyncTelemetry.traceExecution(apparatusName, operationName, async () => {
      ...
      OmnisyncTelemetry.verbose(apparatusName, 'episodic_sync', `Fragmento guardado para: ${sessionIdentifier}`);
    });
  }
  ...
  public static async getSovereignContext(
    ...
    return await OmnisyncTelemetry.traceExecution(apparatusName, 'getSovereignContext', async () => {
      ...
      const rawHistory: unknown[] = await OmnisyncMemory.getHistory(sessionIdentifier, 20);
      
      /**
       * @section Rehidratación Segura (Safe Cast)
       * Erradicamos el 'any' implícito del .map() mediante tipado de parámetro.
       */
      const formattedHistory: IConversationFragment[] = rawHistory.map((item: unknown) => {
   ...
   */
  private static detectDialogueSentiment(content: string): IConversationFragment['sentiment'] {
    const text = content.toLowerCase();
    
    const urgencyPatterns = ['urgente', 'ahora', 'ayuda', 'error', 'falla', 'malo'];
    const positivePatterns = ['gracias', 'perfecto', 'bien', 'genial', 'excelente'];

    if (urgencyPatterns.some(pattern => text.includes(pattern))) return 'URGENT';
    if (positivePatterns.some(pattern => text.includes(pattern))) return 'POSITIVE';
