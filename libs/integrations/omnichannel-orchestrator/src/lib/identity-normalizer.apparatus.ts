...
import { INormalizedIdentity, NormalizedIdentitySchema } from './schemas/identity-normalizer.schema';
...
export class IdentityNormalizerApparatus {
   ...
   */
  public static executeWhatsAppNormalization(rawWhatsAppIdentifier: string): INormalizedIdentity {
    const apparatusName = 'IdentityNormalizerApparatus';

    return OmnisyncTelemetry.traceExecutionSync(apparatusName, 'normalizeWhatsApp', () => {
      
      const isGroupIdentifier = rawWhatsAppIdentifier.includes(this.WHATSAPP_SUFFIXES.GROUP);
      const isSystemBroadcast = rawWhatsAppIdentifier.includes(this.WHATSAPP_SUFFIXES.BROADCAST);
      ...
      return NormalizedIdentitySchema.parse({
        ...
        accountType: isGroupIdentifier ? 'GROUP' : isSystemBroadcast ? 'SYSTEM' : 'INDIVIDUAL',
        sourceChannel: 'WHATSAPP',
        isPotentiallyValidPhone: cleanIdentifier.length >= 8 && cleanIdentifier.length <= 15,
        networkMetadata: {
  ...
  public static executeGenericNormalization(
    ...
    return OmnisyncTelemetry.traceExecutionSync(apparatusName, `normalizeGeneric:${channel}`, () => {
