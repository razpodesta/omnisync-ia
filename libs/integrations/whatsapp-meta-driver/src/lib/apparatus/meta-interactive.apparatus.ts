...
export class MetaInteractiveApparatus {
  ...
  public static buildInteractivePayload(data: IMetaOutboundPayload): unknown {
    ...
    return {
      ...
      header: data.interactiveData?.headerText
        ? { type: 'text', text: data.interactiveData.headerText }
        : undefined,
      body: { text: data.content },
      footer: data.interactiveData?.footerText
        ? { text: data.interactiveData.footerText }
        : undefined,
      action: isList
        ? {
