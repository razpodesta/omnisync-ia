 ...
 */
export const OdooConnectionConfigurationSchema = z
  .object({
    baseUrl: z.string().url({ message: 'URL de instancia Odoo inválida.' }),
    databaseName: z
      .string()
      .min(1, { message: 'Nombre de base de datos requerido.' }),
    userLoginEmail: z
      .string()
      .email({ message: 'Email de autenticación inválido.' }),
    secretApiKey: z
      .string()
      .min(1, { message: 'Odoo API Key es obligatoria.' }),
  })
  .readonly();
...
export type IOdooConnectionConfiguration = z.infer<
  typeof OdooConnectionConfigurationSchema
>;
