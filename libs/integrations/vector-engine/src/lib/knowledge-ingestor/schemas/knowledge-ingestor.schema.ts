 ...
 */
export const KnowledgeIngestionPipelineSchema = z.object({
  rawContent: z.string().min(100, { message: 'integrations.vector_engine.errors.content_too_short' }),
  documentTitle: z.string().min(5).max(255),
  tenantOrganizationIdentifier: TenantIdSchema,
}).readonly();

export type IKnowledgeIngestionPipeline = z.infer<typeof KnowledgeIngestionPipelineSchema>;
